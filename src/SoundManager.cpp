...
#include "headers.h"  // Remove this include file if your project doesn't use precompiled headers.
...
#define DISTANCE_FACTOR       15.0
#define ROLLOFF_SCALE         1.0
...
void SoundManager::Initialize(void)
   ...
   result = system->setFileSystem(&fmodFileOpenCallback, &fmodFileCloseCallback, &fmodFileReadCallback, &fmodFileSeekCallback, nullptr, nullptr,2048);
   if (result != FMOD_OK)
      OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR, "FMOD error! (" + StringConverter::toString(result) + "): " + FMOD_ErrorString(result), "SoundManager::Initialize");
...
void SoundManager::FrameStarted(Ogre::Camera *listenerNode, Ogre::Real timeElapsed)
   ...
   Ogre::Vector3 worldPosition = Ogre::Vector3::ZERO; 
   Ogre::Quaternion worldOrientation = Ogre::Quaternion::IDENTITY;
   if(listenerNode)
   {
	   worldPosition = listenerNode->getPosition();
	   worldOrientation = listenerNode->getOrientation();
   } 

   if (timeElapsed > 0)
	   vectorVelocity = (worldPosition - prevListenerPosition) / timeElapsed;
   else
      vectorVelocity = Vector3(0, 0, 0);

   vectorForward = -worldOrientation.zAxis();
   vectorForward.normalise();

   vectorUp = worldOrientation.yAxis();
   vectorUp.normalise();

   listenerPosition.x = worldPosition.x;
   listenerPosition.y = worldPosition.y;
   listenerPosition.z = worldPosition.z;
   ...
   prevListenerPosition = worldPosition;
   ...
   for (channelIndex = 0; channelIndex < MAX_SOUND_CHANNELS; channelIndex++)
      ...
      if (channelArray[channelIndex].sceneNode != NULL)
		 ...
		 Ogre::Vector3 worldPosition = channelArray[channelIndex].sceneNode->convertLocalToWorldPosition(Ogre::Vector3::ZERO);

         if (timeElapsed > 0)
            vectorVelocity = (worldPosition - channelArray[channelIndex].prevPosition) / timeElapsed;
         else
            vectorVelocity = Vector3(0, 0, 0);
         ...
         channelArray[channelIndex].prevPosition = worldPosition;
         }
      }
   }
...
FMOD_RESULT SoundManager::fmodChannelEndCallback(FMOD_CHANNEL *channel, FMOD_CHANNEL_CALLBACKTYPE type, void *commanddata1, void *commanddata2)
{
	if(type == FMOD_CHANNEL_CALLBACKTYPE_END)
	{
		int index;
		((FMOD::Channel*)channel)->getIndex(&index);
		ChannelInstance* instance = &SoundManager::getSingleton().channelArray[index];
		instance->soundCallback();
	}
	return FMOD_OK;
}
...
void SoundManager::PlaySound(int soundIndex, SceneNode *soundNode, const Ogre::Vector3& location, int *channelIndex,float volume,const std::function<void()>& callback)
   {
   ...
   channel->setCallback(fmodChannelEndCallback);

   channel->getIndex(&channelIndexTemp);
   channelArray[channelIndexTemp].sceneNode = soundNode;
   channelArray[channelIndexTemp].soundCallback = callback;
   channelArray[channelIndexTemp].location = location;

   if (soundNode)
      {
		  
		 Ogre::Vector3 worldPosition = soundNode->convertLocalToWorldPosition(Ogre::Vector3::ZERO);
   ...
   else if(location != Ogre::Vector3::ZERO)
   {
      channelArray[channelIndexTemp].prevPosition = location;
   ...
   //channel->set3DMinMaxDistance(0.0f, 1000.0f);
...
void SoundManager::StopAll3DSounds(void)
{
	int            channelIndex;
	FMOD_RESULT    result;
	FMOD::Channel *nextChannel;
	...
	{
		result = system->getChannel(channelIndex, &nextChannel);
		if(channelArray[channelIndex].sceneNode || channelArray[channelIndex].location != Ogre::Vector3::ZERO)
		{
			if ((result == FMOD_OK) && (nextChannel != NULL))
				nextChannel->stop();
			channelArray[channelIndex].Clear();
...
void SoundManager::SetChannelVolume(int channelIndex,float volume)
   {
   if (channelIndex == INVALID_SOUND_CHANNEL)
      return;
