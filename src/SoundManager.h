...
public:
   ...
   std::function<void()> soundCallback;
   };

inline void DefaultSoundCallback()
{
}
static const std::function<void()> defaultSoundCallback(DefaultSoundCallback);
...
public:
   ...
   void					StopAll3DSounds(void);
   void                 FrameStarted(Ogre::Camera *listener, Ogre::Real timeElapsed);
   ...
   void                 PlaySound(int soundIndex, SceneNode *soundNode, const Ogre::Vector3& location, int *channelIndex,float volume,const std::function<void()>& callback);
...
private:
   ...
   static FMOD_RESULT F_CALLBACK  fmodChannelEndCallback(FMOD_CHANNEL *channel, FMOD_CHANNEL_CALLBACKTYPE type, void *commanddata1, void *commanddata2);
