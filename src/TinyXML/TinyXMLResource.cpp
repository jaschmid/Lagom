...
TinyXMLResource& TinyXMLResource::operator=(const TinyXMLResource& rhs)
{
   ...
   if(rhs.mTinyXMLDoc.Error())
      mTinyXMLDoc.SetError(rhs.mTinyXMLDoc.ErrorId(),rhs.mTinyXMLDoc.ErrorDesc(),nullptr,TIXML_DEFAULT_ENCODING);
...
void TinyXMLResource::loadImpl()
{
   ...
   if (getCreator()->getVerbose())
      LogManager::getSingleton().logMessage("TinyXMLResource: Loading XML data from " + mName + " COMPLETE.");
}
                                  ...
                                  const String& newGroup)
{
   ...
   // copy error id if occured
   if(mTinyXMLDoc.Error())
      newTinyXMLResource->mTinyXMLDoc.SetError(mTinyXMLDoc.ErrorId(),mTinyXMLDoc.ErrorDesc(),nullptr,TIXML_DEFAULT_ENCODING);
